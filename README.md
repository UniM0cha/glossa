...
## 개발 환경 — Python (단일 venv)

`prototype/` 과 `server/` 는 **저장소 루트의 단일 venv 하나**(`.venv/`)를 공유합니다.
(예전엔 디렉토리마다 따로 뒀으나 통합했습니다.)
...
#   루트 requirements.txt 가 server/ · prototype/ 매니페스트를 모두 포함합니다.
```

이후 어떤 컴포넌트를 돌리든 루트 venv 하나만 활성화합니다:

```bash
source .venv/bin/activate            # 저장소 루트에서 한 번 활성화
# 서버:       cd server    && uvicorn app:app --port 8000
# 프로토타입:  cd prototype && python milestone_a.py
```

> Railway 배포는 `server/` 를 빌드 컨텍스트로 `server/requirements.txt` 를 그대로 사용하므로,
> 로컬 venv 통합과 무관합니다.
