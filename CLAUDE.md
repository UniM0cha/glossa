...
## 브랜치 / 배포 (중요)
- **`main` 에 직접 push 금지.** 모든 작업은 `develop` 브랜치에서 하고, 커밋·push 도 develop 으로 한다.
  작업 브랜치가 develop 이 아니면 먼저 `git checkout develop`.
- **`main` = 배포 브랜치.** Railway production 서비스가 `main`(Root Directory `server/`)에 연결돼 있어
  **`main` 에 push/머지되는 순간 서버가 자동 재배포**된다(auto deploy ON). 그래서 평소 main 을 건드리지 않는다.
- **"배포하자" 는 요청을 받았을 때만** main 으로 내보낸다. main 은 develop 의 배포 지점을 그대로 가리키는
  **fast-forward 미러**다 (`--no-ff` 쓰지 않음 → 머지 커밋 없이 linear 유지). develop 에만 커밋하고
  main 에 직접 커밋하지 않으므로 항상 FF 된다:
  ```bash
  git checkout main && git merge --ff-only develop && git push origin main
  git checkout develop   # 배포 후 다시 develop 으로 복귀
  ```
  이 `git push origin main` 이 곧 배포 트리거다. 그 외 상황에서 main 에 머지·push 하지 않는다.

## 작업 시작 전 (항상)
- **모든 작업을 시작하기 전 반드시 `git fetch` 후 `git pull`(현재 브랜치 최신화)로 원격 최신을 먼저 받는다.**
  로컬 미커밋 변경이 있으면 `git stash` → `git pull --ff-only` → `git stash pop` 순서로 보존하며 갱신한다.
  뒤처진 채 작업하면 머지 충돌·중복 작업이 생긴다.
...
### Python 환경 — 루트 단일 venv (규칙)
- **`prototype/` 와 `server/` 는 저장소 루트의 단일 venv(`.venv/`) 하나만 사용한다.** 디렉토리별
  (`server/.venv`·`prototype/.venv`) venv 를 새로 만들지 않는다.
- 설치: 루트에서 `uv pip install -r requirements.txt` — 루트 `requirements.txt` 가
  `server/requirements.txt`·`prototype/requirements.txt` 를 모두 `-r` 로 포함한다. 활성화는 `source .venv/bin/activate`.
- 이 venv 는 **uv 로 생성**되어 `pip list` 가 비어 보일 수 있다(메타데이터 미기록) — 정상이며, 관리는 `uv pip` 로 한다.
- Railway 배포는 `server/` 를 빌드 컨텍스트로 `server/requirements.txt` 를 그대로 쓰므로(원격 빌드) 로컬 venv 통합과 무관하다.
